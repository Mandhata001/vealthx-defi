...
module vealthfx::vault_test {
    ...
    #[test(account = @0x42)]
    public fun test_auto_route(account: &signer) {
        // Initialize vault first
        vault::init_vault(account);
        
        // Test auto-routing (should print "Merkle" and "10" to debug)
        vault::auto_route(account);
        
        // Vault state should remain unchanged (routing is just decision-making for now)
        let (collateral, borrowed) = vault::get_vault(signer::address_of(account));
        assert!(collateral == 0, 1);
        assert!(borrowed == 0, 2);
    }
