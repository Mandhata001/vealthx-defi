...
import { queryGraphQL, POOL_ACTIVITY_QUERY, getVaultAnalytics } from '../lib/aptos';
...
export default function PoolStats() {
  ...
  const REAL_TIME_ENABLED = import.meta.env.VITE_ENABLE_REAL_TIME_UPDATES === 'true';
  ...
  const fetchRealTimeData = async () => {
    ...
    try {
      ...
      const totalVolume = activities.reduce((sum, activity) => 
        sum + parseFloat(activity.amount || 0), 0
      );
      
      const deposits = activities.filter(a => a.activity_type.includes('Deposit'));
      const withdraws = activities.filter(a => a.activity_type.includes('Withdraw'));
  ...
  const fetchOrderBookData = async () => {
    ...
    try {
      ...
      const mockOrderBook = [
        { price: 8.42, size: 1250, side: 'buy', pool: 'Echelon-APT', timestamp: Date.now() },
        { price: 8.41, size: 850, side: 'buy', pool: 'Hyperion-APT', timestamp: Date.now() - 1000 },
        { price: 8.43, size: 950, side: 'sell', pool: 'Merkle-APT', timestamp: Date.now() - 500 },
        { price: 8.44, size: 1100, side: 'sell', pool: 'Echelon-APT', timestamp: Date.now() - 2000 },
        { price: 8.40, size: 1500, side: 'buy', pool: 'Hyperion-APT', timestamp: Date.now() - 3000 },
      ];
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <span className={`px-2 py-1 rounded text-xs font-medium ${
                bestPool.risk === 'Low' ? 'bg-green-400 text-green-900' :
                bestPool.risk === 'Medium' ? 'bg-yellow-400 text-yellow-900' :
          ...
          <div className="mt-4 pt-4 border-t border-green-400">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-semibold">{realTimeData.totalVolume.toFixed(2)} APT</p>
              </div>
              <div>
                <p className="text-green-100">Activity Rate</p>
                <p className="font-semibold">{realTimeData.activityRate} tx/h</p>
              </div>
              <div>
                <p className="text-green-100">Avg Size</p>
                <p className="font-semibold">{realTimeData.avgTransactionSize.toFixed(2)} APT</p>
              </div>
              <div>
                <p className="text-green-100">Last Update</p>
                <p className="font-semibold">{new Date(lastUpdate).toLocaleTimeString()}</p>
              </div>
            </div>
          </div>
        )}
      ...
      {ORDER_BOOK_ENABLED && orderBookData.length > 0 && (
        <div className="bg-white rounded-xl p-6 shadow-sm border">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 Live Order Book</h3>
          <div className="space-y-2">
            {orderBookData.slice(0, 5).map((order, index) => (
              <div key={index} className="flex justify-between items-center py-2 px-3 rounded bg-gray-50">
            ...
            <div key={index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold">{pool.volume24h?.toFixed(0) || 'N/A'} APT</p>
                </div>
                <div>
      ...
      {realTimeData && (
        <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
          <h3 className="text-lg font-semibold text-blue-900 mb-4">📈 Live Network Stats</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{realTimeData.totalDeposits}</p>
              <p className="text-sm text-blue-700">Total Deposits</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{realTimeData.totalWithdraws}</p>
              <p className="text-sm text-blue-700">Total Withdraws</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{realTimeData.totalVolume.toFixed(0)}</p>
              <p className="text-sm text-blue-700">Volume (APT)</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{realTimeData.activityRate}</p>
